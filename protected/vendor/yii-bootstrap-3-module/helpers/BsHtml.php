...
class BsHtml extends CHtml
{
    ...
    { 
        ...
        // 20150620/Samuel
        // 如果添加的是button，则使用input-group-btn
...
EOD;
     ...
     * @return type
     */
    public static function activeUploadPicControlGroup($model, $attribute, $htmlOptions = array()){
        $opt = array('append'=>self::button("上传", array("color"=>"white", "id"=>"btn-upload-pic-{$attribute}", "class"=>"btn-upload-pic")));
        return self::activeControlGroup(self::INPUT_TYPE_TEXT, $model, $attribute, array_merge($htmlOptions, $opt));
    }
    
    public static function activeUploadAudioControlGroup($model, $attribute, $htmlOptions = array()){
        $opt = array('append'=>self::button("上传", array("color"=>"white", "id"=>"btn-upload-audio-{$attribute}", "class"=>"btn-upload-audio")));
        return self::activeControlGroup(self::INPUT_TYPE_TEXT, $model, $attribute, array_merge($htmlOptions, $opt));
    }
    
    public static function activeUploadVideoControlGroup($model, $attribute, $htmlOptions = array()){
        $opt = array('append'=>self::button("上传", array("color"=>"white", "id"=>"btn-upload-video-{$attribute}", "class"=>"btn-upload-video")));
        return self::activeControlGroup(self::INPUT_TYPE_TEXT, $model, $attribute, array_merge($htmlOptions, $opt));
    }    
    
    public static function activeUploadFileControlGroup($model, $attribute, $htmlOptions = array()){
        $opt = array('append'=>self::button("上传", array("color"=>"white", "id"=>"btn-upload-file-{$attribute}", "class"=>"btn-upload-file")));
        return self::activeControlGroup(self::INPUT_TYPE_TEXT, $model, $attribute, array_merge($htmlOptions, $opt));
    }
     ...
     * @param type $htmlOptions
     */
    public static function activeUploadPicsControlGroup($model, $attribute, $htmlOptions = array()){
        // 使用hidden input保存真正的内容
        // 图片上传组件上传完图片后必须更新此hidden input
        $hidden = parent::activeHiddenField($model, $attribute);
        $input = $hidden . "<div><a class='btn btn-white btn-upload-pics' id='btn-upload-pics-" . $attribute . "' style='display:block;height:150px;width:150px;float:left;margin-right:10px;margin-bottom:10px;'><span class='glyphicon glyphicon-plus' style='color:#ccc;font-size:20px;font-size:50px;line-height:130px;'></span></a></div>";
        return self::customActiveControlGroup($input, $model, $attribute, $htmlOptions);
    }
    
    /**
     * 20150622/Samuel/生成日期组件，需要配合第三方js组件才能真正生效
     * 原来的方法activeDateFieldControlGroup()只是将input的type设置为date，需要html5的支持才能展示为日期控件，在pc浏览器上几乎是无效的，因此增加此方法，通过第三方js组件实现日期控件
     * @param type $model
     * @param type $attribute
     * @param type $htmlOptions
     * @return type
     */
    public static function activeDateControlGroup($model, $attribute, $htmlOptions = array()){
        $opt = array('class'=>'datetime-input','id'=>'datetime-input-' . $attribute, 'data-format'=>'YYYY-MM-DD',
                     'append'=>BsHtml::icon('glyphicon-calendar'));
        return self::activeControlGroup(self::INPUT_TYPE_TEXT, $model, $attribute, array_merge($htmlOptions, $opt));
    }
    
    /**
     * 20150622/Samuel/生成时间组件，需要配合第三方js组件才能真正生效
     * @param type $model
     * @param type $attribute
     * @param type $htmlOptions
     * @return type
     */
    public static function activeTimeControlGroup($model, $attribute, $htmlOptions = array()){
        $opt = array('class'=>'datetime-input','id'=>'datetime-input-' . $attribute, 'data-format'=>'HH:mm:ss',
                     'append'=>BsHtml::icon('glyphicon-time'));
        return self::activeControlGroup(self::INPUT_TYPE_TEXT, $model, $attribute, array_merge($htmlOptions, $opt));
    }
    
    /**
     * 20150622/Samuel/生成日期+时间组件，需要配合第三方js组件才能真正生效
     * @param type $model
     * @param type $attribute
     * @param type $htmlOptions
     * @return type
     */
    public static function activeDatetimeControlGroup($model, $attribute, $htmlOptions = array()){
        $opt = array('class'=>'datetime-input','id'=>'datetime-input-' . $attribute, 'data-format'=>'YYYY-MM-DD HH:mm:ss',
                     'append'=>BsHtml::icon('glyphicon-calendar'));
        return self::activeControlGroup(self::INPUT_TYPE_TEXT, $model, $attribute, array_merge($htmlOptions, $opt));
    }
     ...
     * @param type $htmlOptions
     */
    public static function activeStringsControlGroup($model, $attribute, $htmlOptions = array()){
        // 使用hidden input保存真正的内容
        // 后边的文本框进行更新时必须更新此hidden input
        $hidden = parent::activeHiddenField($model, $attribute);
        $opt = array('class'=>'strings-input', 'rows' => 6);
        $input = $hidden . self::textArea("", "", $opt);
        return self::customActiveControlGroup($input, $model, $attribute, $htmlOptions);
    }
     ...
     * @return type
     */
    public static function activeChoiceControlGroup($model, $attribute, $data = array(), $htmlOptions = array()){
        // 使用hidden input保存真正的内容
        // 后边的inline checkbox group进行选择时必须更新此hidden input
        $hidden = parent::activeHiddenField($model, $attribute);
        $opt = array('containerOptions'=>array('class'=>'choice-input'));
        $input = $hidden . self::inlineCheckBoxList("choice-input-{$attribute}", "", $data, $opt);
        return self::customActiveControlGroup($input, $model, $attribute, $htmlOptions);
    }
    ...
    public static function activeDropDownList($model, $attribute, $data, $htmlOptions = array())
    {
          ...
          //modified by mihe 2015-06-20, the old code is follow.  
//        self::addCssClass('form-control', $htmlOptions);
